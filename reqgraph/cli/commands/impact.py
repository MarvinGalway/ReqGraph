...
§9.2 step 4/6, models-config `impact_traversal_rules`) then impact_analyst
classification. Never sets Contract.knowledge_status; writes an audit record
under /.project-state/impact/ (an operational extension, not a new node
label — see plan Ambiguity 7).
"""
...
def run(
    ...
    with graph_session() as sess:
        ...
        candidates = gather(sess, target_id, label, depth=depth)
        if not candidates.contracts:
            console.print(f"[yellow]{label} {target_id} implements/constrains no Contract — nothing to assess.[/yellow]")
            raise typer.Exit(code=0)

        contract_lines = [
            f"{c.id}: pre={c.preconditions} post={c.postconditions}" for c in candidates.contracts
        ]
